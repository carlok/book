...
For a planar mechanism, the classical Gr\"ubler--Kutzbach mobility count gives the generic mobility
\begin{equation}
M = 3(n - 1) - 2j_1 - j_2,
\end{equation}
...
Each one-degree-of-freedom joint corresponds locally to a one-parameter subgroup of the relevant Euclidean motion group. Planar joints can be treated through $\mathfrak{se}(2)$; in spatial notation the same idea is represented through the exponential map
\[
...
Third, the full mechanism is described by algebraic or analytic equations obtained from loop closure. These equations define the configuration variety or configuration manifold of the linkage, depending on the chosen coordinates and on the regularity and singularity structure of the constraints.
