\chapter{Geometric Rigidity and Algebraic Symmetries of Complex Transcendental Numbers}

\begingroup

% Local notation copied from the standalone source.
...
\newcommand{\PeriodicPoints}{\operatorname{Per}}
...
\section{Introduction}\label{grct:sec:introduction}
...
%The paper is organized as follows. Section~\ref{grct:sec:defining-domain} establishes the structural foundations, defining the automorphism group and the real forms of the space. Section~\ref{grct:sec:group-actions} analyzes the global dynamics, proving the invariance of the transcendental locus and the topological transitivity of the action. Section~\ref{grct:sec:symmetric-generic} introduces the geometric hierarchy, formally defining the flexible and generic loci. Section~\ref{grct:sec:geometric-interpretations} provides a detailed geometric and dynamical analysis of the flexible locus, including the non-smoothness of the orbit relation. Sections~\ref{grct:sec:constructing-seeds} and \ref{grct:sec:tools} provide explicit constructions of transcendental seeds and the auxiliary dynamical tools underpinning our density results.

\section{Structural foundations: automorphisms and real forms}\label{grct:sec:defining-domain}
...
\begin{definition}[Algebraic generalized circles]\label{grct:def:alg-generalized-circle}
...
\begin{lemma}[Rigidity of intersections]\label{grct:lem:alg-intersection}
...
\begin{lemma}[Algebraic normalization of generalized circles]\label{grct:lem:circle-normalization}
...
\subsection{Geometric generators}\label{grct:subsec:inversive-input}

The structure of $\Gamma^{\pm}$ is closely related to the classical inversive geometry of circle reflections.

\begin{remark}[Classical two-reflection picture]\label{grct:rem:two-reflections}
...
\subsection{Rigidity of the algebraic structure}\label{grct:subsec:max-transfer}
...
\begin{definition}[Transfer functions]\label{grct:def:transfer-semigroup}
...
\begin{lemma}[Three algebraic points determine algebraic coefficients]\label{grct:lem:three-pts}
...
\begin{theorem}[Maximality as structural rigidity]\label{grct:thm:maximal-transfer}
...
\end{theorem}
\begin{proof}
	Fix $T\in\mathcal S$. Since $T(\PAlg)\subseteq\PAlg$, in particular $T$ maps the three algebraic projective points $\infty,0,1$ to algebraic projective points. By Lemma~\ref{grct:lem:three-pts}, $T\in\Gamma^{\pm}$. Hence $\mathcal S\subseteq\Gamma^{\pm}$. Maximality follows because $\Gamma^{\pm}$ itself has this property.
\end{proof}

\begin{remark}[Real forms do not replace the skeleton]\label{grct:rem:real-forms-skeleton}
	The family of algebraic generalized circles is useful because its intersections recover algebraic projective points (Lemma~\ref{grct:lem:alg-intersection}). Still, a bare statement such as ``$T$ maps some algebraic generalized circles to algebraic generalized circles'' is not, by itself, a clean replacement for the hypothesis $T(\PAlg)\subseteq\PAlg$. To force $T\in\Gamma^{\pm}$ one needs enough incidence data to know that three distinct algebraic projective points are sent to algebraic projective points; then Lemma~\ref{grct:lem:three-pts} applies. For this reason, the maximality theorem above is stated in terms of the projective algebraic skeleton itself.
\end{remark}

\begin{remark}[Justification of the scope]\label{grct:rem:why-g}
...
\section{Global dynamics: invariance and transitivity}\label{grct:sec:group-actions}
...
\begin{lemma}[No holomorphic stabilizers on $\Tc$]\label{grct:lem:no-holo-stab}
...
\begin{theorem}[Invariance of the transcendental locus]\label{grct:thm:preservation-transcendence}
...
\begin{theorem}[Transcendence preserved by $\Gamma^{\pm}$]\label{grct:thm:preservation-transcendence-anti}
	If $z\in \Tc$ and $g\in\Gamma^{\pm}$, then $g(z) \in \Tc$.
\end{theorem}
\begin{proof}
	The holomorphic case is Theorem~\ref{grct:thm:preservation-transcendence}. In the anti-holomorphic case, write $g=h\circ\J$ with $h\in\Gamma$. Complex conjugation preserves $\Alg$, so it also preserves $\Tc$; applying the holomorphic case to $h$ finishes the proof.
\end{proof}
...
\begin{lemma}[Dense affine algebraic orbits]\label{grct:lem:dense-affine-orbits}
...
\begin{corollary}[Topological transitivity]\label{grct:cor:topological-transitivity}
...
\end{corollary}
\begin{proof}
	Choose $x\in U$ and $y\in V$. Since $V$ is open, Lemma~\ref{grct:lem:dense-affine-orbits} gives an affine algebraic map $T$ with $T(x)\in V$, and hence $T(U)\cap V\ne\varnothing$. If $U,V$ are relatively open in $\Tc$, choose $x\in U$ and use the density of the orbit in $\C$ together with the invariance of $\Tc$ under $\Gamma$.
\end{proof}

\section{The geometric hierarchy: flexible, rigid, and generic loci}\label{grct:sec:symmetric-generic}
...
\begin{definition}[Stabilizers and the flexible Locus]\label{grct:def:stabilizer}
...
\begin{lemma}[Reflections from Hermitian forms]\label{grct:lem:R_H}
...
\begin{proposition}[Flexible geometry of circles]\label{grct:prop:lines-circles-general}
	Every transcendental point on an algebraic generalized circle (including lines, where $A=0$, and proper circles, where $A\ne0$) is symmetric. Its stabilizer contains the reflection $R_H$ defined in Lemma~\ref{grct:lem:R_H}.
\end{proposition}
\begin{proof}
	If $z\in\Tc$ lies on the circle $\mathcal C_H$, then Lemma~\ref{grct:lem:R_H} gives an element $R_H\in\Gamma^{\pm}$ with $R_H(z)=z$. Since $R_H$ is anti-holomorphic, it is not the identity. Hence the stabilizer of $z$ is non-trivial.
\end{proof}
...
\begin{example}[Canonical axes]\label{grct:ex:real-imag}
...
\begin{example}[Algebraic circles]\label{grct:ex:algebraic-circles}
...
\begin{lemma}[Symmetry implies algebraic dependence]\label{grct:lem:symm-dependence}
...
\begin{definition}[Independence locus]\label{grct:def:ind-locus}
	...
	\]
	By Lemma~\ref{grct:lem:symm-dependence}, we have $\sind\subseteq\sgen$.
\end{definition}

\begin{lemma}[Invariance of the independence locus]\label{grct:lem:ind-locus-invariant}
...
\begin{proposition}[Genericity of the independence locus]\label{grct:prop:ind-locus-generic}
...
\begin{corollary}[Dense orbits in the independence locus]\label{grct:cor:ind-locus-dense-orbits}
...
\end{corollary}
\begin{proof}
	By Lemma~\ref{grct:lem:ind-locus-invariant}, the orbit stays in $\sind$. By Lemma~\ref{grct:lem:dense-affine-orbits}, the $\Gamma$-orbit is dense in $\C$, hence it meets every non-empty open subset of $\C$ whose intersection with $\sind$ is non-empty.
\end{proof}

Thus, while the flexible locus is dense, the generic geometry of $\Tc$ is dominated by the rigid independence locus.

\section{Geometry and dynamics of the flexible locus}\label{grct:sec:geometric-interpretations}
...
\begin{remark}[Algebraic generalized circles: equivalent descriptions]\label{grct:rem:equiv-alg-circle}
...
\begin{lemma}[Anti-M\"obius stabilizers force a circle]\label{grct:lem:anti-fix}
...
\end{lemma}
\begin{proof}
	The square $g^2$ is holomorphic and belongs to $\Gamma$. Since $g(t)=t$, we also have $g^2(t)=t$. By Lemma~\ref{grct:lem:no-holo-stab}, no non-identity element of $\Gamma$ can fix a transcendental point. Hence $g^2=\id$, so $g$ is an anti-Möbius involution.
...
\begin{lemma}[Countably many algebraic circles]\label{grct:lem:countable-circles}
...
\begin{theorem}[Structure of the flexible locus]\label{grct:thm:symm-locus}
...
\end{theorem}
\begin{proof}
	Suppose first that $z\in\ssymm$. Then some non-identity $g\in\Gamma^{\pm}$ fixes $z$. By Lemma~\ref{grct:lem:no-holo-stab}, $g$ cannot be holomorphic. Hence $g$ is anti-holomorphic, and Lemma~\ref{grct:lem:anti-fix} places $z$ on an algebraic generalized circle.
	
	Conversely, if $z\in\Tc$ lies on an algebraic generalized circle $C=\mathcal C_H$, then Lemma~\ref{grct:lem:R_H} gives an anti-Möbius reflection $R_H\in\Gamma^{\pm}$ fixing every point of $C$, in particular $z$. Thus $z$ is flexible. Countability follows from Lemma~\ref{grct:lem:countable-circles}.
\end{proof}

\begin{proposition}[Footprint of the flexible locus]\label{grct:prop:flexible-footprint}
	The flexible locus $\ssymm$ is dense in $\C$, meagre, Lebesgue null, and has Hausdorff dimension $1$.
\end{proposition}
\begin{proof}
	By Theorem~\ref{grct:thm:symm-locus} and Lemma~\ref{grct:lem:countable-circles}, $\ssymm$ is contained in a countable union of generalized circles. Each such circle is closed with empty interior in $\C$, has planar Lebesgue measure zero, and has Hausdorff dimension at most $1$. Hence $\ssymm$ is meagre, null, and has Hausdorff dimension at most $1$.
...
\begin{proposition}[Uniqueness of the stabilizing reflection]\label{grct:prop:unique-reflection}
...
\end{proposition}
\begin{proof}
	If two anti-Möbius elements $R$ and $S$ both fixed $z$, then $RS$ would be holomorphic and would fix $z$. Lemma~\ref{grct:lem:no-holo-stab} forces $RS=\id$, hence $R=S$. Thus the stabilizing reflection is unique.
	
	If $z$ lay on two distinct algebraic generalized circles, the two corresponding reflections from Lemma~\ref{grct:lem:R_H} would both fix $z$, contradicting uniqueness. Equivalently, two distinct algebraic generalized circles meet only in algebraic points by Lemma~\ref{grct:lem:alg-intersection}.
\end{proof}

\begin{remark}[Notation for flexible coordinates]\label{grct:rem:CtRt}
	For any $t\in\ssymm$, we denote by $C(t)$ the unique algebraic generalized circle passing through $t$ (as guaranteed by Proposition~\ref{grct:prop:unique-reflection}), and by $R_t$ the unique anti–Möbius reflection such that $\Fix(R_t)=C(t)$.
\end{remark}

\begin{lemma}[Transport of flexible geometry]\label{grct:lem:G-preserves-circles}
...
\begin{lemma}[Coordinate field trap on a flexible curve]\label{grct:lem:coordinate-field-trap}
...
\begin{theorem}[Perfect antichains on flexible curves]\label{grct:thm:perfect-antichain}
...
\begin{proof}
	...
	By Lemma~\ref{grct:lem:circle-normalization}, normalize $C$ to the unit circle $S^1$ by some $M\in\PGL(2,\Alg)$. Smoothness is preserved under this conjugacy, so it suffices to argue on $S^1\cap\Tc$.
...
\section{Constructing seeds in the transcendental locus}\label{grct:sec:constructing-seeds}
...
\begin{theorem}[Lindemann--Weierstrass generation]\label{grct:thm:lindemann-weierstrass}
...
\begin{theorem}[Linear–exponential seeds]\label{grct:thm:linear-exponential-seed}
...
\begin{proof}
	...
	\]
	is an algebraic linear relation among exponentials of distinct algebraic numbers. By Theorem~\ref{grct:thm:lindemann-weierstrass}, all coefficients in this relation must vanish. This contradicts the assumption that some $c_j$ with $j\ge1$ is non-zero.
\end{proof}

\section{Auxiliary dynamical tools}\label{grct:sec:tools}
...
\begin{lemma}[Möbius difference formula]\label{grct:lem:mob-difference}
...
\begin{theorem}[Explicit $\varepsilon$-net construction]\label{grct:thm:epsilon-net}
...
\begin{proposition}[Coarse expansion/compression]\label{grct:prop:coarse-expansion}
...
\begin{corollary}[Affine mixing]\label{grct:cor:affine-semigroup-transitive}
...
\end{corollary}
\begin{proof}
	Choose $x\in U$ and $y\in V$. Since $V$ is open and the affine algebraic orbit of $x$ is dense by Lemma~\ref{grct:lem:dense-affine-orbits}, there is $T\in\mathcal A$ with $T(x)\in V$.
\end{proof}
...
\begin{theorem}[Invariance of $\mu$ under rational Möbius]\label{grct:thm:irr-exp-invariance}
...
\end{theorem}
\begin{proof}
	After clearing denominators, assume $a,b,c,d\in\Z$. For rationals $p/q$ sufficiently close to $\xi$, the denominator $cp+dq$ is comparable to $q$, and Lemma~\ref{grct:lem:mob-difference} gives
	\[
...
\begin{remark}[Generation by flexible reflections]\label{grct:rem:gen-by-reflections}
	If one invokes the classical two-reflection factorization from Remark~\ref{grct:rem:two-reflections}, then the extended group is generated by reflections across the symmetric locus:
	\[
		\Gamma^{\pm}=\langle\, R_y\ :\ y\in\ssymm\,\rangle.
	\]
	Every algebraic generalized circle contains transcendental points, and by Proposition~\ref{grct:prop:unique-reflection} all such points on the same circle determine the same reflection. Thus the notation $R_y$ ranges over the algebraic circle reflections.
...
The chapter reframes complex transcendental numbers through the degree-one
algebraic geometry of the projective line. The resulting picture separates a
flexible locus, governed by algebraic generalized circles and anti-Möbius
reflections, from a generic locus with trivial stabilizers and dense orbits.
This does not replace classical transcendence theory, but it adds a geometric
language for organizing how algebraic symmetries can and cannot act on
