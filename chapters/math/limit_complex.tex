...
\begin{abstract}
We examine the limit $\lim_{z \to a} \frac{z^n - a^n}{z - a}$ where $z, a, n \in \C$. For non-integral exponents, this expression is not a single complex-valued quotient until a local branch of $z^n$ has been fixed near $a$. Unlike the real case, the complex setting introduces fundamental complications: multi-valued functions, branch cuts, path dependence, and holomorphicity conditions. We provide a systematic treatment of all cases and subtleties.
\end{abstract}

\section{Introduction}

This chapter uses a familiar difference quotient to expose why complex
exponentiation is more delicate than its real counterpart. The expression is
important not because the final formula is surprising, but because reaching it
safely requires choosing branches, tracking monodromy, and distinguishing local
holomorphic behavior from global multivaluedness. It therefore serves as a
compact entry point into the book's recurring theme: simple symbolic formulas
often hide geometric and analytic structure.
...
\label{eq:main_limit}
\end{equation}
For $n \notin \Z$, this notation is shorthand for the corresponding difference
quotient on a fixed single-valued branch of $z^n$ defined near $a$. Without that
choice, the numerator is multivalued rather than a single complex-valued
function.
...
\begin{itemize}
    ...
    \item $a \in (-\infty, 0)$: On the principal branch cut
    \item $a \in \C \setminus (-\infty, 0]$: In the principal branch domain
\end{itemize}
...
\begin{theorem}
Let $n = p/q \in \Q \setminus \Z$. Fix a branch of $z^n$ that is holomorphic on a neighborhood of $a \in \C \setminus \{0\}$. Then:
\begin{equation}
\lim_{z \to a} \frac{z^n - a^n}{z - a} = n a^{n-1},
\end{equation}
...
\begin{warning}
If $a \in (-\infty, 0)$ (on the branch cut for the principal branch), the principal branch does not define $a^n$ at $a$ itself. The quotient is therefore not intrinsically defined unless one prescribes a boundary value or switches to a branch whose domain contains $a$:
\begin{itemize}
    \item The principal branch has different boundary values from the two sides of the cut.
    \item Any boundary-value convention can make the result depend on the \textbf{path of approach}.
...
\begin{cases}
0 & \text{if } p/q > 1, \\
\text{no finite limit in } \C & \text{if } 0 < p/q < 1.
\end{cases}
\end{equation}
The excluded value $p/q = 1$ would be an integer exponent, contrary to the
standing assumption $n \in \Q \setminus \Z$.
\end{proposition}
...
\begin{proposition}
If $n$ is irrational and real, then $z^n$ has \textbf{infinitely many branches}.
\end{proposition}

\begin{theorem}
Fix a branch of $z^n$ (e.g., the principal branch). For $a$ in the domain of that branch:
\begin{equation}
\lim_{z \to a} \frac{z^n - a^n}{z - a} = n a^{n-1}.
\end{equation}
\end{theorem}
...
\begin{theorem}
For $n \in \C$ and $a$ in the domain of a fixed branch:
\begin{equation}
\lim_{z \to a} \frac{z^n - a^n}{z - a} = n a^{n-1}.
\end{equation}
\end{theorem}
...
\begin{theorem}
Let $n \notin \Z$, and let $a = 0$ or let $a$ lie on a chosen branch cut. The expression
\begin{equation}
\lim_{z \to a} \frac{z^n - a^n}{z - a}
\end{equation}
is not intrinsically defined as a limit of a single-valued holomorphic function.
After an endpoint or boundary-value convention is imposed, the resulting limit
...
\begin{proposition}
If $n = a \neq 0$ (the exponent equals the base point) and $a$ lies in the domain of the chosen branch:
\begin{equation}
...
\begin{proposition}
If $n = 1/a$ and $a \neq 0$ lies in the domain of the chosen branch:
\begin{equation}
...
\begin{proof}
This is the definition of the complex derivative. If $\Lambda$ is the chosen
branch of the logarithm and $z^n = e^{n\Lambda(z)}$, then:
\begin{equation}
\frac{d}{dz} e^{n\Lambda(z)} = e^{n\Lambda(z)} \cdot \frac{n}{z} = z^n \cdot \frac{n}{z} = n z^{n-1}.
\end{equation}
\end{proof}
...
\begin{proposition}
A single-valued global function $f(z) = z^n$ is holomorphic on $\C \setminus \{0\}$ if and only if $n \in \Z$.
...
Let $n \in \C$, and let $U \subset \C \setminus \{0\}$ be a domain on which a
single-valued branch of $z^n$ has been chosen. For any $a \in U$:
\begin{equation}
\boxed{\lim_{z \to a} \frac{z^n - a^n}{z - a} = n a^{n-1}.}
\end{equation}
Here both $a^n$ and $a^{n-1}$ are evaluated using the same branch.
\end{theorem}
